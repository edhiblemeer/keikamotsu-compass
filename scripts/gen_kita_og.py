"""tokyo-kita OG image generator.
hero_v3.jpg (1536x1024) を 1200x630 に crop (上端寄せ・横はcenter) し、
右側に dark gradient + "KITA" (white serif) + "KEIKAMOTSU RANKING 2026" (sans) を overlay。
nerima/itabashi og.jpg と同トーン。 raw も og_raw.jpg として保存。
テキストは gpt-image-1 で焼かず PIL ImageDraw で載せる (memory: feedback_og_image_text_overlay)。
"""
from PIL import Image, ImageDraw, ImageFont

SRC = "public/images/area/tokyo-kita/hero_v3.jpg"
OUT = "public/images/area/tokyo-kita/og.jpg"
OUT_RAW = "public/images/area/tokyo-kita/og_raw.jpg"

OG_W, OG_H = 1200, 630

# serif for big title (英字), sans for subtitle
SERIF = "C:/Windows/Fonts/timesbd.ttf"  # Times New Roman Bold (serif, NERIMA/ITABASHI と同系)
...
def crop_cover(im: Image.Image, w: int, h: int) -> Image.Image:
    ...
    # 横center・縦は上から0.30寄せ → nerima/itabashi 近似
    left = (new_w - w) // 2
...
def main() -> None:
    ...
    title_font = ImageFont.truetype(SERIF, 150)
